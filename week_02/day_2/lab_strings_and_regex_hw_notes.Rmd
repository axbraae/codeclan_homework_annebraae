...
```{r}
whole_text %>% 
  str_detect("[Tt]hought [Pp]olice")

#could at a + inbetween thought police to check for one or more spaces - more robust
```
Yes, the Thought Police is mentioned at least once.

2. Count how often Big Brother is mentioned

```{r}
pattern2 <- "[Bb][Ii][Gg] [Bb][Rr][Oo][Tt][Hh][Ee][Rr]"

whole_text %>% 
  str_count(pattern2)

#can wrap regex pattern in reg ex helper function and ignore_case = TRUE
...
whole_text %>%
  ...
  length()
```
Big Brother is mentioned 20 times.

3. The chapter is written from the point of view of the protagonist, Winston. Does he ever mention the other main character (Julia) by name?

```{r}
whole_text %>% 
  str_detect("[Jj]ulia")
```
No, Julia is not mentioned by name in this chapter/text.
...
```{r}
pattern4 <- "\\w+\\s\\d+\\w+,\\s\\w+"

#(add the + after the d to catch more than one number)

#this also works...why? "\\w+ \\d\\w+, \\w+"

whole_text %>% 
  str_extract(pattern4)

#the below is more robust
...
```{r}
#Note: used str_detect(winston) and str_detect(WINSTON) to check for different case occurances of Winston. Also used str_detect(Smith) to see that his last name was used.
pattern5 <- "Winston"
pattern5lastname <- " Smith"
  
whole_text_redacted <- whole_text %>% 
  str_replace_all(pattern5lastname, "") %>% 
  str_replace_all(pattern5, "REDACTED")

whole_text_redacted

#check that Winston is mentioned!!

#check that there are no winstons mentioned
whole_text %>%
  str_detect(pattern5)

# check that redacted is mentioned
pattern5c <- "REDACTED"
whole_text %>%
  str_detect(pattern5c)
```
...
```{r}
#Find all mentions of Victory Gin using str_detect() and substituting one of the below patterns.

pattern6upper  <- "VICTORY GIN"
pattern6title <- "Victory Gin" #not found in text 
pattern6lower <- "victory gin" #not found in text 
pattern6single <- "gin"


whole_text %>%
  str_detect(pattern = pattern6upper)

whole_text %>%
  str_detect(pattern = pattern6title) #not found in text 

whole_text %>%
  str_detect(pattern = pattern6lower) #not found in text 

whole_text %>%
  str_detect(pattern = pattern6single)


#replace text which was found with required substitute
vodka_whole_text <- whole_text %>% 
  str_replace_all(pattern6upper, "VODKA") %>% 
  str_replace_all(pattern6single, "vodka")


# check case where gin appears (want to make sure is the word gin, and not 'gin' in the middle of another word (e.g. beginning)
pattern <- "[ ,.-][gG][iI][nN][ ,.-]"
pattern <- "\\b[gG][iI][nN]\\b"
...
```{r}
#Again, before running the str_replace_all, I checked for occurrences of each word with str_detect() as for question 6.

rewrite <- whole_text %>% 
  str_replace_all("[dD]epartment", "Section") %>% 
  str_replace_all("[mM]inistry", "Department") %>%
  str_replace_all("[mM]inistrie", "Department") %>% 
  str_replace_all("[mM]ini", "Dep")
...
2. Return form which expression exists in
3. Iterate through text to replace all forms with required replacements.
4. Check replacements have been made

Write a function to return true if expression exists and continue, or false and stop.
Take in word to change and what you want it to be changed to.
