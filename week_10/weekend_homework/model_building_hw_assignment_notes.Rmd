...
Note can remove highly correlated variables such as x4225 and x4770.
...
```{r}
# run forwards
regsubsets_forwards <- regsubsets(average_price ~ ., data = avocados, nvmax = 57, method = "forward")
...
```{r}
# run backwards
regsubsets_backwards <- regsubsets(average_price ~ ., data = avocados, nvmax = 57, method = "backward")
...
Hooray! The residual diagnostics look pretty good! The residuals vs. fitted fall around 0 indicating equal variances, the residuals have a normal distribution as apart from very high and very low values, all values fall on or close to the line in the QQ plot. The variances are homoscedastic having similar variances (seen on the scale - location plot).


The predictive model I have developed in terms of main effects is: `average_price ~ type + year + season + region`, I have not considered interaction terms. The model explains 62% of the variance of `average_price` and has a standard error of 0.2482.
...
The average error for the final model is 0.2486 and the average r-squared for the final model is 0.61859.
This means the model is able to account for 61.8% of the variance in the `average_price` of avocados with a standard error of 0.2486. My model suggests that avocado `type` as organic or conventional, what year they were sold in, what season they are sold and what region they are sold in all contribute to the `average_price` of the avocados.

validation:
check significance
check r squared increased
analysis of variance if some levels unsignificant
