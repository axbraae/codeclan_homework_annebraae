...
The p-value of 6e-04 is less than the alpha of 0.05. There is enough evidence in the sample data to reject the null hypothesis. That is that the proportion of domesticated animal types in the population of animal types is statistically significantly greater than 5%.

NOTE: NEVER ACCEPT HA only say there is enough evidence to support the alternative hypothesis. OR say accept that and then copy the HA text. CANNOT ACCEPT HA CAN ONLY REJECT OR FAIL TO REJECT HO. CAN ACCEPT alternative hypothesis in this sampled data. We don't know that the hypothesis will always be like this.
...
Notes: If sample size is huge need to set critical value lower to compensate for multiple testing.
Notes: p-value: a measure of the probability that an observed difference could have occurred just by random chance.
