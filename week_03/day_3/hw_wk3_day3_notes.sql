...
FROM employees
GROUP BY department
ORDER BY COUNT(id) DESC NULLS LAST --need to order on COUNT(employee(id))
LIMIT 1
)
SELECT
	e.id,
...
FROM employees
GROUP BY department
ORDER BY department DESC NULLS LAST -- SHOULD ORDER ON COUNT(id)
...
--WINDOWS FUNCTION SOLUTION - biggest dept NO ties 
...
SELECT
	...
	salary,
	fte_hours,
	salary/AVG(salary) OVER () AS ratio_salary_to_dept_salary,
	fte_hours/AVG(fte_hours) OVER () AS ratio_fte_hours_to_dept_hours
FROM employees 
WHERE department = (
	SELECT 
	department 
...
-- ORDER() in the above are partitioning on all windows, 
-- the data has already been filtered by the WHERE subquery.
...
SELECT 
id, first_name, last_name, department,
AVG(salary) OVER() AS dept_avg_salary -- now taking average of ALL depts
FROM employees 
ORDER BY id;
