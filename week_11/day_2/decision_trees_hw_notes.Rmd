...
titanic_set_clean <- titanic_set %>% 
  ...
  mutate(
    sex = as.factor(sex),
    survived_flag = factor(survived, levels = c(0,1),
                      labels = c("No","Yes")),
  ...
  select(-c("X1", "passenger_id", "name", "ticket", "fare", "cabin", "embarked", "pclass", "age", "survived")) %>% 
    na.omit()
```
...
titanic_set_clean %>% 
  ggplot(aes(x = survived_flag, fill = embarkation)) +
  geom_bar(position = "dodge")
