...
# can also make test train split with caret::createDataPartition
...
After 16950 models:
Best model: purchase_mm ~ loyal_ch + sale_price_mm + sale_price_ch + store_id_7
...
Note for glmulti 1 just means add the intercept! remove this from the models...

The best model was: `purchase_mm ~ loyal_ch + sale_price_mm + sale_price_ch + store_id_7`
...
glmulti_search_previous_mains_one_pair <- glmulti(
  purchase_mm ~ loyal_ch + sale_price_mm + sale_price_ch + store_id_7, 
  data = juice_train,
...
The best interaction model is: `purchase_mm ~ loyal_ch + sale_price_mm + sale_price_ch + store_id_7 + sale_price_mm:loyal_ch + store_id_7:sale_price_ch`.
...
```{r}
model_all <- glm(purchase_mm ~ loyal_ch + sale_price_mm + sale_price_ch + store_id_7, data = juice_train, family = binomial(link = "logit"))

model_all_int <- glm(purchase_mm ~ loyal_ch + sale_price_mm + sale_price_ch + store_id_7 + sale_price_ch:sale_price_mm + store_id_7:sale_price_ch, data = juice_train, family = binomial(link = "logit"))
...
The model with out the interactions has the higher AUC (0.907 compared to 0.904). Therefore the model without interactions is marginally better than the model with theh interactions based on their AUC values.
